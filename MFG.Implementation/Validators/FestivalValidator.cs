...
namespace MFG.Implementation.Validators
{
    public class FestivalValidator : AbstractValidator<FestivalDto>
    {
        ...
        public FestivalValidator(MFGContext context)
        {
            _context = context;
            ...
            RuleFor(x => x.PlaceID)
                ...
                .WithMessage("PlaceID field is required")
                 ...
                 {
                     RuleFor(x => x.Id)
                     .Must((t, p) => !_context.Festivals.Any(f => f.Name.ToLower() == t.Name.ToLower() && f.StartDate == t.StartDate && f.EndDate == t.EndDate))
